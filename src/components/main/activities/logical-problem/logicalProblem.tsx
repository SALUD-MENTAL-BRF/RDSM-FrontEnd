...
//   const [selectStage, setSelectStage] = useState<string>("");
  ...
  const [responseAI, setResponseAI] = useState<logicalProblemResponseDto>({
    problema: "",
    correcta: "",
    respuestas: [],
    explicacion:""
  ...
  const [settingState, setSettingState] = useState<logicalProblemSettingDto>();
...
//   };
  ...
  const initGame = async () => {
    ...
    const response = await fetchLogicalProblem(settingState)
  ...
  useEffect(() => {
    ( 
      async () => {
        const setting:logicalProblemSettingDto = await CustomFetch(`${import.meta.env.VITE_API_URL}logical-problem/setting/${professionalId}/${patientId}`, 'GET');
  ...
  const saveResponse = async () => {
    ...
    const save = await fetch(`${import.meta.env.VITE_API_URL}logical-problem/history/${professionalId}/${patientId}`,{
      method:'POST',
      body: JSON.stringify({
        problem:responseAI.problema,
  ...
  return (
    ...
    <h1 className="text-center mb-4">Resolución de problemas</h1>
        ...
        {!activityState ? (
          <div className="container-initActivity">
            <div className="row g-3">
              ...
              <div className="col-md-6">
                <select className="form-select" disabled>
                  <option>{settingState ? settingState.theme : "Configuración del profesional"}</option>
                </select>
              </div>
              <div className="col-md-6">
                <select className="form-select" disabled>
                  <option>{settingState ? settingState.problem_type : "Configuración del profesional"}</option>
                </select>
              </div>
          ...
          <div className="text-center">
            ...
            <h6 className="card-text mb-4">{responseAI.problema}</h6>
